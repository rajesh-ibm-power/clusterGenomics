\name{plotTreeRow}
\alias{plotTreeRow}
\title{Add a row dendrogram to a heatmap.}
\description{Plots a row dendrogram to the left of a heatmap created with \code{plotHeatmap}.}
\usage{
plotTreeRow(clust, groups, margin)
}
\arguments{
  \item{clust}{output from call to \code{hclust} to cluster the rows in the heatmap.}
  \item{groups}{optional vector of the same length as the number of rows in the heatmap, and with values specifying the colors of the leaves in the dendrogram.}
...
\details{This method is designed to be used in conjunction with the method \code{\link{plotHeatmap}}. The argument value \code{margin} should match the one used in the call to \code{plotHeatmap}; for details on how to use this argument, see the help file for that method. The default values are the same as in \code{plotHeatmap}, hence if no \code{margin} is specified in the call to \code{plotHeatmap}, then no \code{margin} needs to be specified in the call to \code{plotTreeRow} either.
}


\seealso{\code{\link{plotHeatmap}}, \code{\link{plotColorbarCol}}, \code{\link{plotColorbarRow}}, \code{\link{plotTreeCol}}, \code{\link{plotColorRange}}}
